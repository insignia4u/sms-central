	...
	Datepicker.prototype = {
		...
		fill: function() {
			...
			while(prevMonth.valueOf() < nextMonth) {
				...
				clsName = '';
				if ((prevMonth.getMonth() < month || (month == 0 && prevMonth.getMonth() == 11)) && !(month == 11 && prevMonth.getMonth() == 0)) {
					clsName += ' old';
				} else if (prevMonth.getMonth() > month || (month == 11 && prevMonth.getMonth() == 0)) {
					clsName += ' new';
				}
				...
				switch(target[0].nodeName.toLowerCase()) {
					case 'th':
						switch(target[0].className) {
							...
							case 'prev':
							case 'next':
                                date = this.viewDate.getDate();
                                month = this.viewDate['get'+DPGlobal.modes[this.viewMode].navFnc].call(this.viewDate) + 
									DPGlobal.modes[this.viewMode].navStep * (target[0].className == 'prev' ? -1 : 1);
                                year = this.viewDate.getFullYear();
                                dim = DPGlobal.getDaysInMonth(year,month);
                                if(date > dim){
                                    this.viewDate.setDate(dim);
                                }
